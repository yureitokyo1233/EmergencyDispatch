...
public class DispatchSystem {
    ...
    public void reportIncident(int section, int severity, int type, String medicalCondition) {
        // 🕒 Capture current time
        LocalTime now = LocalTime.now();

        // 🟢 Add new incident directly (merge handled later when user edits the table)
        int newId = incidents.size() + 1;
        Incident newIncident = new Incident(newId, section, severity, type, medicalCondition);
        incidents.add(newIncident);

        System.out.println("New incident added: " + newIncident.medicalCondition);
    }
    ...
    public void setIncidents(List<Incident> updatedList) {
        incidents.clear();
        incidents.addAll(updatedList);
    }
}