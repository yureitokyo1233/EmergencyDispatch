...
public class main {
    ...
    public static void main(String[] args) {
        ...
        Image bgImage = new ImageIcon(System.getProperty("user.home") + "/Downloads/map2.jpg").getImage();
        ...
        chatPanel.setBackground(new Color(0, 0, 0, 180)); // translucent
        ...
        chatInput.addActionListener(e -> {
            ...
            if (ChatHashmap.codes.containsKey(input)) {
                ...
                try {
                    ...
                    triggerDispatch(system, section, type, severity, incidentTable, tableModel, mapPanel, returnBtn, frame);
                } catch (Exception ex) {
        ...
        MouseAdapter dragAdapter = new MouseAdapter() {
            public void mousePressed(MouseEvent e) {
                clickOffset.setLocation(e.getPoint());
            }

            public void mouseDragged(MouseEvent e) {
                JLayeredPane parent = (JLayeredPane) chatPanel.getParent();
                Point mouseLoc = SwingUtilities.convertPoint(chatPanel, e.getPoint(), parent);

                int newX = mouseLoc.x - clickOffset.x;
                int newY = mouseLoc.y - clickOffset.y;

                // Keep panel inside map bounds
                newX = Math.max(0, Math.min(newX, parent.getWidth() - chatPanel.getWidth()));
                newY = Math.max(0, Math.min(newY, parent.getHeight() - chatPanel.getHeight()));

                chatPanel.setLocation(newX, newY);
            }
        };

        // Attach drag listener to chat panel AND all children so dragging works even on text area
        chatPanel.addMouseListener(dragAdapter);
        chatPanel.addMouseMotionListener(dragAdapter);
        chatArea.addMouseListener(dragAdapter);
        chatArea.addMouseMotionListener(dragAdapter);
        chatInput.addMouseListener(dragAdapter);
        chatInput.addMouseMotionListener(dragAdapter);
        ...
        mapPanel.setBounds(0, 0, 1280, 800);
        chatPanel.setBounds(900, 500, 350, 250);
        ...
        tabs.addTab("Map + Chat", layeredPane);
